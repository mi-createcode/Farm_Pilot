...
def get_policy_text():
    if 'city' in st.session_state and 'town' in st.session_state:
        prompt = f"{st.session_state['city']} 정책"
        st.subheader(f"📍 {st.session_state['city']} {st.session_state['town']} 정책/보조금 정보")
        with st.spinner("정책 정보를 불러오는 중... ⏳"):
            try:
                completion = openai_compliance(prompt)
